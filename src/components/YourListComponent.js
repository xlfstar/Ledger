...
class FlatListWithCustomRefresh extends Component {
  ...
  componentDidMount() {
    this.initScrollViewRefresh = DeviceEventEmitter.addListener(
      'initScrollViewRefresh',
      () => {
        this.onRefreshEnd()
      }
    )
        ...
        <View
          ...
          onLayout={(e) => {
            ...
            if (
              ...
              e.nativeEvent.layout.height < height
            ) {
              this.viewRef &&
                this.viewRef.setNativeProps({
                  style: { height: height - e.nativeEvent.layout.height },
                })
            } else {
              this.viewRef &&
                this.viewRef.setNativeProps({ style: { height: 0 } })
            }
          }}
        >
